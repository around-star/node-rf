...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    times = torch.arange(0,args.render_num)/50
    times_obs = times[:1]
    #time_to_pred = render_times*5
    times_to_pred = times
    c2w = render_poses[0]
       
    ## Bifurcating hill 2D GT
...
def create_nerf(args):
    ...
    latent_ode_model = create_LatentODE_model(args, args.latent_dim, z0_prior, 0.01, device, num_frames=args.num_frames+1)
...
def train():
    ...
    if args.render_only or True:
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}_loc_pos_vel_5x_extrap'.format('test' if args.render_test else 'path', start))
