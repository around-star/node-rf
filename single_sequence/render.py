...
def render_path_extrapolate(args, render_poses, rnn_model, render_times, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None,
    ...
    time_obs = times[:args.num_obs]
    time_to_pred = times#[0:][::1]
    # time_to_pred = render_times
    c2w = render_poses[-1]
    ...
    print("No. of Latents", latent_nexts.shape)
...
def config_parser():
    ...
    parser.add_argument('--config', is_config_file=True, default='./configs/bouncingballs.txt',
    ...
    parser.add_argument('--num_obs', type=int, default=2, help="Number of intial observations")
    ...
    parser.add_argument('--ode_type', type=str, default='dopri5', choices=['dopri5', 'euler', 'adams', 'rk4'])
    ...
    parser.add_argument('--ckpt_path', default='/home/guests/hiran_sarkar/Final_Code/single_sequence_bouncing_ball/logs_bouncing_balls/logs_bouncing_balls_n2f6_iego_render/bouncingballs/105000.tar')
...
def train():
    ...
    if args.dataset_type == 'blender':
        ...
        # dataloaders, times, render_poses, render_times, hwf, bds = load_llff_data(args, args.datadir,  start_frame=0, end_frame=12, factor=2, device=device) ## DynamicSceneDataset 
    ...
    if args.render_only or True:
        print('RENDER ONLY')
        nerf_state_dict = torch.load(args.ckpt_path)
        ...
        render_kwargs_test["network_fn"].load_state_dict(nerf_coarse_state_dict)
        ...
        with torch.no_grad():
            ...
            testsavedir = os.path.join(basedir, expname, 'renderonly_{}_{:06d}_ckpt'.format('test' if args.render_test else 'path', start))
            os.makedirs(testsavedir, exist_ok=True)
            print('test poses shape', render_poses.shape)

            rgbs, _ = render_path_extrapolate(args, render_poses, vid_ode_model, render_times, hwf, args.chunk, render_kwargs_test, gt_imgs=images,
