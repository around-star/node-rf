...
def create_nerf(args):
    ...
    # Create optimizer
    optimizer = torch.optim.Adam(params=grad_vars, lr=args.lrate_nerf, betas=(0.9, 0.999))

    
    # optimizer = torch.optim.Adam([
    # {'params': grad_vars_nerf, 'lr': args.lrate_nerf},
...
def config_parser():
    ...
    parser.add_argument("--lrate_nerf", type=float, default=5e-4, 
                        help='learning rate')
    parser.add_argument("--lrate_node", type=float, default=5e-4,
                       help='learning rate')
    ...
    parser.add_argument('--sample_size', type=int, default=150, help="Number of time points to sub-sample")
    ...
    parser.add_argument('--dataset_type', type=str, default='blender', choices=['blender', 'dynerf'])
...
def train():
    ...
    if args.dataset_type == 'blender':
        ...
        dataloaders, times, render_poses, render_times, hwf, bds = load_llff_data(args, args.datadir, final_height=338, skip=2, device=device)
    ...
    for i in trange(start, N_iters):
        ...
        if i < 5000 or (i % 4000 >= 2000 and i % 4000 <= 2200):
        # if False:
